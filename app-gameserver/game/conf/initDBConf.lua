...
-- 刷库
function initDBConf:executeGameDataSql()
    gLog.i("initDBConf:executeGameDataSql begin")
    if dbconf.dbtype == "mongodb" then
    
    elseif dbconf.dbtype == "mysql" then
        local sql = io.readfile("sql/mysql/gamedata.sql")
        -- gLog.d("initDBConf:executeGameDataSql sql=", sql)
        local address = require("svrAddrMgr").getSvr(svrAddrMgr.gameDBSvr)
        local ret = skynet.call(address, "lua", "execute", sql)
        assert(ret and not ret.err, "initDBConf:executeGameDataSql error, ret="..table2string(ret))
    end
    gLog.i("initDBConf:executeGameDataSql end")
end
