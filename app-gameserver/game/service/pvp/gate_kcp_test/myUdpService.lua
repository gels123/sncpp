...
local function server()
	...
	host = socket.udp(function(str, from)
		gLog.i("server1 recv str=", str, "from=", socket.udp_address(from))
  ...
  --       gLog.i("=====server1 while=====", current, "diff=", diff)
  --       if diff > 0 then
  --           -- LUtil.isleep(diff)
...
local function client()
	gLog.i("== myUdpService client ==")
	local client1 = nil
	local c = socket.udp(function(str, from)
		gLog.i("client recv str=", str, socket.udp_address(from))
	...
	local function udp_output(buf)
		if socket then
			...
			gLog.i("client udp_output dont buf=", buf)
		end
	end
	--此处info用于output接口回调数据
    local session = 0x11223344
    ...
    while 1 do
        ...
        gLog.i("=====client1 while===== current=", current, "diff=", diff)
        if diff > 0 then
            -- LUtil.isleep(diff)
        ...
        while current >= slap do
            ...
            gLog.i("=====client1 lkcp_send index=", index, "current=", current, "str=", str, #str, "package=", package)
		...
		while 1 do
            ...
            gLog.i("client recv true sn=", sn, "ts=", ts)

            local rtt = current - ts
			
			if sn ~= inext then
				--如果收到的包不连续
				gLog.i(string.format("ERROR sn %d<->%d\n", count, inext))
				return
            end
			...
			gLog.i(string.format("[RECV] mode=%d sn=%d rtt=%d\n", mode, sn, rtt))
        end
		if inext > 1 then
            break
        end
