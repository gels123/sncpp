...
local function recv_package(server1, last)
    ...
    if result then
        gLog.i("server1 r1==", hrlen, "r=", r, "e==")
        return result, last
    end
    local hrlen, r = server1:lkcp_recv()
    --没有收到包就退出
    if hrlen <= 0 then
        gLog.i("server1 r2==", hrlen, "r=", r, "e==")
        return nil, last
    end
    gLog.i("server1 r3==", hrlen, "r=", r, "e==")
    if r == "" then
        gLog.i("===server1 Server closed")
        error "Server closed"
    end
...
local function client()
    gLog.i("== mykcpcli begin ==")
    local sid, uid = 0,  1889771
    local kcp = nil
    local current = getms()
    c = socket.udp(function(str, from)
        local clisubid, str2 = unpack_package(str)
        gLog.i("client recv str=", str, socket.udp_address(from), "clisubid=", clisubid, "str2=", str2)
        if sid == 0 then
            if clisubid > 0 then
                gLog.i("client recv str login ok=", str, socket.udp_address(from), "clisubid, str2=", clisubid, str2)
                sid = clisubid
                kcp = createKcp(sid)
                ...
                skynet.fork(function()
                    ...
                    while 1 do
                        ...
                        --gLog.i("=====client kcp while===== current=", current, "diff=", diff)
                        if diff > 0 then
                        ...
                        while current >= slap do
                            ...
                            gLog.i("=====client lkcp_send index=", index, "current=", current, "args=", args, "package=", package, "#package=", #package)
                            ...
                            --gLog.i("=====client lkcp_send index2=", index, "t=", t, "name=", name, "args=", args, "response=", response, "package=", package, "#package=", #package)
                ...
                gLog.i("client recv str login error=", str, socket.udp_address(from), "clisubid, str2=", clisubid, str2)
            end
        else
            kcp:lkcp_input(str2)
            current = getms()
            ...
            gLog.i("cli xxx777fdfdfad data==", hrlen, r)
            if hrlen > 0 then
                local t, name, args = host:dispatch(r)
                gLog.i("cli xxx777fdfdfad data dispatch==", t, name, args)
            end
        end
    end)
