...
local function recv_package(kcp, last)
    ...
    if result then
        gLog.i("recv_package do1 result==", result, "last=", last)
        return result, last
    end
    local hrlen, r = kcp:lkcp_recv()
    --没有收到包就退出
    if hrlen <= 0 then
        gLog.i("server recv_package do2 hrlen==", hrlen, "r=", r)
        return nil, last
    end
    gLog.i("server recv_package do3 hrlen==", hrlen, "r=", r)
    if r == "" then
        gLog.i("recv_package do4 hrlen==", hrlen, "r=", r)
        error "Server closed"
    end
    result, last = unpack_package(last .. r)
    gLog.i("server recv_package do5 result==", result, "last=", last)
    return result, last
end

local function server()
    gLog.i("== mykcpsvr begin ==")
    local last = ""
    local host = nil
    local client = nil
    local kcp  = nil
    ...
    local function udp_output(buf, sid)
        if host and client then
            ...
            gLog.i("server udp_output error buf=", buf)
        end
    end
    local function createKcp(sid)
        gLog.i("server createKcp sid=", sid)
    ...
    local function dispatch_msg2(str, from)
        gLog.i("server kcp recv str=", str, "from=", socketdriver.udp_address(from), "[end]", from, "[end]")
        ...
        if clisubid == 0 then
            ...
            gLog.i("server kcp recv client connet in, sid=", sid, playerid)
            ...
            local hrlen, r = kcp:lkcp_recv()
            gLog.i("server kcp recv xxx777fdfdfad data==", hrlen, r)
        end
    end
    ...
    --   local nextt1 = kcp:lkcp_check(current)
    --   local diff = nextt1 - current
    --   gLog.i("=====kcp while=====", current, "diff=", diff)
    --   if diff > 0 then
    --       skynet.sleep(math.ceil(diff/10)) -- lutil.isleep(diff)
    --       current = getms()
    --   end
    ...
    --            break
    --        end
    --        gLog.i("kcp recv xxxxxgels v=", v, "[end]")
    --    end
    --end
...
local function server2()
    ...
    local s = require("iSockServer").new()
    s:init(3, 8765)
