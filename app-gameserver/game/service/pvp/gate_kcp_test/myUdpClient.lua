...
local function client()
    local c = socket.udp(function(msg, from)
        gLog.d("client recv1, msg=", msg, "from=", socket.udp_address(from))
        local str, offset = string.unpack(">s2", msg)
        gLog.d("client recv2, str=", str, "offset=", offset)
        local type, name, args, response = host:dispatch(str, offset)
        gLog.d("client recv3, type=", type, "name=", name, "args=", args, "response=", response)
    end)
    ...
    while(num < 999999) do
        ...
        if num == 1 then
            ...
            gLog.d("=============client send====msg=", msg, "size=", string.len(msg))
        elseif num == 2 then
            local args = {
                uid = 12900,
                batId = 10086,
        ...
        --gLog.d("=============34534543535345====xxx=", type, name, args, response)
