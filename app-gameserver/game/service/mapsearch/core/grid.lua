...
-- 获取地形
function Grid:getLand(node)
  return self._map[node._y] and self._map[node._y][node._x]
end

function Grid:isWalkable(x, y, x0, y0)
  if self._map[y] and self._map[y][x] then
    if x0 and y0 then
      local land = self._map[y0] and self._map[y0][x0]
      if (land == 0 or self._map[y][x] == 0 or land == self._map[y][x]) then
        return true
      end
    else
      return true
...
function Grid:getNeighbours(node)
  ...
  for i,offset in pairs(straightOffsets) do
    ...
    if n and self:isWalkable(n._x, n._y, node._x, node._y) then
  ...
  for i,offset in pairs(diagonalOffsets) do
    ...
    if n and self:isWalkable(n._x, n._y, node._x, node._y) then
      ...
      if (n1 and n2) and not self:isWalkable(n1._x, n1._y, node._x, node._y) and not self:isWalkable(n2._x, n2._y, node._x, node._y) then
      else
        neighbours[#neighbours+1] = n 
      end
    end
