...
local mapObjMill = class("mapObjMill", mapObjectCell)

-- override
function mapObjMill.get_db_fields()
    local db_fields = mapObjMill.super.get_db_fields()
    table.merge(db_fields, {
        __table__ = "mapobjectmill",
...
--初始化
function mapObjMill:init(params)
    ...
    --gLog.dump(params, "mapObjMill:init create_object=", 10)
end

function mapObjMill:get_level()
    return self:get_field("level") or 0
end

function mapObjMill:clear_data()
    self:set_field("subtype", 0)
    self:set_field("level", 0)
    self:set_position(0, 0)
end

function mapObjMill:get_config()
