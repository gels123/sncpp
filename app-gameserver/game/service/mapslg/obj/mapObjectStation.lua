...
local mapObjectStation = class("mapObjectStation", mapObjectCell)

-- override
function mapObjectStation.get_db_fields()
    local db_fields = mapObjectStation.super.get_db_fields()
    table.merge(db_fields, {
        __table__ = "mapobjectstation",
...
--初始化
function mapObjectStation:init(params)
    ...
    --gLog.dump(params, "mapObjectStation:init create_object=", 10)
end

function mapObjectStation:get_level()
    return self:get_field("level") or 0
end

function mapObjectStation:clear_data()
    self:set_field("subtype", 0)
    self:set_field("level", 0)
    self:set_position(0, 0)
end

function mapObjectStation:get_config()
    local subtype, level = self:getSubMapType(), self:get_level()
    return get_static_config().treasure[subtype][level]
end

return mapObjectStation