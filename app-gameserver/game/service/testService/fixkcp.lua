...
local function udp_output(buf, info)
    ...
    gLog.d("======fixtest send=", info.id, buf)
    -- lsm:send(info.id, buf)
end

local function test(mode)
	gLog.i("=====fixtest test mode=", mode)
    ...
    local server1 = LKcp.lkcp_create(session, function (buf)
        udp_output(buf, info)
    end)
    local info2 = {
        id = 1,
    ...
    local client1 = LKcp.lkcp_create(session, function (buf)
    ...
    elseif mode == 1 then
		--普通模式，关闭流控等
        server1:lkcp_nodelay(0, 10, 0, 1)
        client1:lkcp_nodelay(0, 10, 0, 1)
    else
		--启动快速模式
        ...
        server1:lkcp_nodelay(1, 10, 2, 1)
        client1:lkcp_nodelay(1, 10, 2, 1)
    end
    ...
    while 1 do
		...
		while 1 do
			...
			--如果 p2收到udp，则作为下层协议输入到client1
			client1:lkcp_input(hr)
        end
		...
		while 1 do
			...
			--如果 p1收到udp，则作为下层协议输入到server1
			server1:lkcp_input(hr)
        end
