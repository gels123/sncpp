...
local mapObjectCommandPost = class("mapObjectCommandPost", mapObjectCell)

-- override
function mapObjectCommandPost.get_db_fields()
    local db_fields = mapObjectCommandPost.super.get_db_fields()
    table.merge(db_fields, {
        __table__ = "mapobjectcommandpost",
        ...
        maxhp = models.NumberField(),          --最大耐久度
...
--初始化
function mapObjectCommandPost:init(params)
    ...
    --gLog.dump(params, "mapObjectCommandPost:init create_object=", 10)
end

function mapObjectCommandPost:get_level()
    return self:get_field("level") or 0
end

function mapObjectCommandPost:clear_data()
    self:set_field("subtype", 0)
    self:set_field("level", 0)
    self:set_position(0, 0)
end

function mapObjectCommandPost:get_config()
    local subtype, level = self:getSubMapType(), self:get_level()
    return get_static_config().treasure[subtype][level]
end

return mapObjectCommandPost