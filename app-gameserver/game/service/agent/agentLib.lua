...
-- send玩家代理(若离线则拉起)
function agentLib:sendAgent(kid, uid, ...)
    self:send(kid, "sendAgent", uid, ...)
end
...
-- 批量通知在线玩家
function agentLib:notifyMsgBatch(kid, uids, cmd, msg, exp)
    self:send(kid, "notifyMsgBatch", uids, cmd, msg, exp)
end
