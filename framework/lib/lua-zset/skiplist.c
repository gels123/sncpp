...
int slIsInRange(skiplist *sl, double min, double max) 
{
    ...
    if (x == NULL || x->score > max)
        return 0;
    // printf("slIsInRange sl=%p min=%f max=%f scoreMin=%f scoreMax=%f\n", sl, min, max, sl->header->level[0].forward->score, sl->tail->score);
    return 1;
}
...
{
    // printf("slFirstInRange sl=%p min=%f max=%f\n", sl, min, max);
    skiplistNode *x;
    int i;
...
{
    // printf("slLastInRange sl=%p min=%f max=%f\n", sl, min, max);
    skiplistNode *x;
    int i;
