...
static inline skiplist* _to_skiplist(lua_State *L)
{
...
static int _delete_by_rank(lua_State *L)
{
    skiplist *sl = _to_skiplist(L);
...
static int _get_count(lua_State *L)
{
    skiplist *sl = _to_skiplist(L);
...
static int _get_rank(lua_State *L)
{
    skiplist *sl = _to_skiplist(L);
    double score = luaL_checknumber(L, 2);
    luaL_checktype(L, 3, LUA_TSTRING);
...
static int _get_rank_range(lua_State *L)
{
    skiplist *sl = _to_skiplist(L);
...
static int _get_score_range(lua_State *L)
{
    skiplist *sl = _to_skiplist(L);
...
static int _get_member_by_rank(lua_State *L)
{
    skiplist *sl = _to_skiplist(L);
...
static int _get_backward_member_by_score(lua_State *L)
{
    skiplist *sl = _to_skiplist(L);
    double s1 = luaL_checknumber(L, 2);
    if (sl->tail == NULL) {
    ...
    skiplistNode *node = slFirstInRange(sl, s1, sl->tail->score);
    if (node == NULL) {
        return 0;
    }
    lua_pushlstring(L, node->obj->ptr, node->obj->length);
...
static int _get_forward_member_by_score(lua_State *L)
{
    skiplist *sl = _to_skiplist(L);
    double s1 = luaL_checknumber(L, 2);
    if (sl->header == NULL || sl->header->level[0].forward == NULL) {
        return 0;
    }
    skiplistNode *node = slLastInRange(sl, sl->header->level[0].forward->score, s1);
    if (node == NULL) {
        return 0;
    }
    lua_pushlstring(L, node->obj->ptr, node->obj->length);
...
{
    skiplist *sl = _to_skiplist(L);
    printf("lua-skiplist.c collect sl:%p\n", (void*)sl);
    slFree(sl);
    return 0;
}
...
int luaopen_skiplist_c(lua_State *L)
{
    ...
    luaL_Reg l[] = 
    {
        ...
        {"get_backward_member_by_score", _get_backward_member_by_score},
        {"get_forward_member_by_score", _get_forward_member_by_score},
