...
function serviceCenterBase:getSq(type)
    ...
    if not self.sq[type] then
        if not self.sqf then
            self.sqf = handler(self, self.delSq)
        end
        if not self.myTimer then
            self.myTimer = require("timerList").new()
            self:start()
        end
        self.sq[type] = {q = skynetQueue(), timer = self.myTimer:createTimer(1800, self.sqf, 1, type)} --防止sq泄漏
    else
        self.myTimer:modifyTime(self.sq[type].timer, 1800)
    end
