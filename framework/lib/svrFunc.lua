...
function svrFunc.getNextWeekDayUTC(nextWeekDay)
    ...
    elseif weekDay > nextWeekDay then
        return sec0 + secOfOneDay * (7 - weekDay + nextWeekDay)
    end
end
...
function svrFunc.getPreWeekDayUTC(preWeekDay)
    ...
    elseif weekDay < preWeekDay then
        return sec0 - secOfOneDay * (7 - preWeekDay + weekDay)
    elseif weekDay > preWeekDay then
...
--[[
    ...
    @param pos 保留小数位
--]]
function svrFunc.round(num, pos)
    if "number" ~= type(num) then
        -- print("svrFunc.round: num is not a number!")
        return num
    end
    pos = pos or 0
    if "number" ~= type(pos) then
        -- print("svrFunc.round: pos is not a number!")
        return num
    end
    if pos < 0 then
        -- print("svrFunc.round: pos is lowwer than 0!")
        return num
    end
    local n = 10^(-pos) / 2
    if num < 0 then
...
--[[合并奖励
        reward结构见sMailExtra定义, 注意dest将被改变
]]
function svrFunc.mergeReward(dest, reward)
    if not dest then
        dest = {}
    end
    -- 合并道具
    if reward.items and next(reward.items) then
        if not dest.items then
            dest.items = {}
        end
        local find = false
        for k,v in pairs(reward.items) do
            if v.id and v.count then
                find = false
                for kk,vv in pairs(dest.items) do
                    if vv.id == v.id then
                ...
                if not find then
                    table.insert(dest.items, {id = v.id, count = v.count})
                end
            end
        end
    end
